...
//die();
    ...
    if ($mrow[0]['parent_id'] > 0) {
                $data = array(
                    ...
                    'txfromid' => $idmbr,
                    'txtoid' => $mrow[0]['parent_id'],
                    'txamount' => 50,
                    'txmemo' => '1st level Commision',
                    'txppid' => 1,
...
// if(isset($mrow[0]['parent_id'])) {
//     //2nd level parent
//     $idmbr = $mrow[0]['parent_id'];
//     $condition = ' AND idmbr='.$idmbr;
//     $mrow = $db->getAllRecords(DB_TBLPREFIX . '_mbrplans', 'idref as parent_id', $condition);
...
//     $idmbr =$mrow[0]['parent_id'];
//     $condition = ' AND idmbr='.$idmbr;
//     $mrow = $db->getAllRecords(DB_TBLPREFIX . '_mbrplans', 'idref as par4ent_id', $condition);
//     print $mrow[0]['parent_id']."-3rd level upper parent\n";
// }
...
//     $condition = ' AND idmbr='.$idmbr;
//     $mrow = $db->getAllRecords(DB_TBLPREFIX . '_mbrplans', 'idref as parent_id', $condition);
//     print $mrow[0]['parent_id']."-4th level upper parent\n";
// }
...
//     $condition = ' AND idmbr='.$idmbr;
//     $mrow = $db->getAllRecords(DB_TBLPREFIX . '_mbrplans', 'idref as parent_id', $condition);
//     print $mrow[0]['parent_id']."-5th level upper parent\n";
// }
//end of test
}
...
if (isset($FORM['dosubmit']) and $FORM['dosubmit'] == '1') {
    ...
    // $condition = " AND idref = ".$refstr['mpid'];

    // var_dump($refstr['mpid']);
    // die();

    // $ref_sql = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_mbrplans WHERE 1 " . $condition . "");
        ...
        <form method="post" action="adduser.php" oninput='password1.setCustomValidity(password2.value != password1.value ? "Passwords do not match." : "")'>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <select name="position" id="position_id">
                        <option value="volvo">Left</option>
                        <option value="saab">Right</option>
                    </select>
                </div>
