    ...
    if ($condition != '') {
        ...
        if ($key == 'unex') {
            ...
            if ($mbrRow['id'] > 0) {
                ...
                $ref_cnt = $db->getAllRecords(DB_TBLPREFIX . '_mbrplans', '*', ' AND idref = "' . $mbrRow['id'] . '"');
                // member status
                ...
                // display member info
                if(count($ref_cnt)<=1)
                 echo $icostatus . "<div class='text-primary nogapline'><strong>{$mbrRow['username']} Children#".count($ref_cnt)."</strong><br /><span class='text-info text-small'>{$mbrRow['firstname']} {$mbrRow['lastname']} ({$mbrRow['email']})</span></div>";
                else echo $icostatus . "<div class='text-primary nogapline'><strong>Pls choose another referer username. This user has already 2 immediate downline users</strong><br /></div>";
            } else {
