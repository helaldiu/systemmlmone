...
function doipnbox($txmpid, $payamount, $paygate, $txbatch, $redirurl, $ipnreturn = '', $skipamount = 0, $addtoken = '') {
    ...
    if (($trxstr['txamount'] <= $txamount || $skipamount == 1) && get_optionvals($trxstr['txtoken'], 'isapproved') != 1) {
        ...
        if ($update && ($newtrxid > 0 || $existingtxstatus == 0)) {
            ...
            $getcmlist = getcmlist($refstr['mpid'], $reflist, $bpprow['cmdrlist'], $mbrstr,$bpprow['clube_bonus'],$bpprow['pro_commission'],$bpprow['dipo_commission'], $bpprow['dilar_commission'], $bpprow['office_rent'],$bpprow['staf_fee'],$bpprow['entertainment_fee']);
            ...
            // Clube Bonus
            $sprstr = getmbrinfo($mbrstr['idspr']);
            $getcmlistcl = getcmlist($sprstr['mpid'], $mbrstr['sprlist'], $bpprow['clube_bonus'], $mbrstr);
            addcmlist('Clube Bonus', 'TIER', $getcmlistcl, $mbrstr, $trxstr);
            ...
            // Promossion Commission
            $sprstrpro = getmbrinfo($mbrstrpro['idspr']);
            $getcmlistpro = getcmlist($sprstrpro['mpid'], $mbrstr['sprlist'], $bpprow['pro_commission'], $mbrstr);
            addcmlist('Promossion Commission', 'TIER', $getcmlistpro, $mbrstrpro, $trxstr);
        }
